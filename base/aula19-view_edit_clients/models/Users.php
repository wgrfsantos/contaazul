...
class Users extends model {
    ...
    public function getList($id_company) {
        ...
        $sql = $this->db->prepare("
            SELECT 
                ...
                users.email,
                users.name,
                permission_groups.name as name_group
            FROM users 
    ...
    public function add($email, $name, $group, $pass, $id_company) {
        ...
        if($row['c'] == '0') {
            $sql = $this->db->prepare("INSERT INTO users SET email = :email, name = :name, password = :password, id_group = :id_group, id_company = :id_company");
            $sql->bindValue(':email', $email);
    ...
    public function edit($email, $name, $group, $pass, $id, $id_company) {
       
        $sql = $this->db->prepare("UPDATE users SET email = :email, name = :name, id_group = :id_group, id_company = :id_company WHERE id = :id");
        $sql->bindValue(':email', $email);
        $sql->bindValue(':name', $name);        
        $sql->bindValue(':id_group', $group);
        $sql->bindValue(':id_company', $id_company);
        ...
        if(!empty($pass)) {
            $sql = $this->db->prepare("UPDATE users SET password = :password, id_company = :id_company WHERE id = :id");
            $sql->bindValue(':password', md5($pass));
            $sql->bindValue(':id_company', $id_company);
            $sql->bindValue(':id', $id);
            $sql->execute();
